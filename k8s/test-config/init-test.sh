...
rancher kubectl delete secret mcda-test-secrets -n drugis-test
rancher kubectl create secret generic mcda-test-secrets \
  -n drugis-test \
...
rancher kubectl delete configmap mcda-test-settings -n drugis-test
rancher kubectl create configmap mcda-test-settings \
  -n drugis-test \
  --from-literal=MCDAWEB_AUTHENTICATION_METHOD=GOOGLE \
  --from-literal=MCDAWEB_DB_USER=mcdatest \
  --from-literal=MCDA_HOST=https://mcda-test.edge.molgenis.org \
  --from-literal=MCDAWEB_DB_HOST=postgres \
  --from-literal=MCDAWEB_DB_NAME=mcdatest \
