  ...
  describe('The WorkspaceService, ', function () {
    ...
    describe('mergeBaseAndSubProblem', function () {
      it('should create a new problem from the original problem with the sub problem merged into it', function () {
        ...
        var result = workspaceService.mergeBaseAndSubProblem(
          problem,
    ...
    describe('buildAggregateState', function () {
      it('should aggregate the problem with the subproblem and the scenario, and set the theoretical scales', function () {
        ...
        var result = workspaceService.buildAggregateState(
          problem,
          subProblem,
          scenario
        );
    ...
    describe('setDefaultObservedScales', function () {
      ...
      it('should set observed scale ranges if none are on the problem & subproblem', function () {
        ...
        var result = workspaceService.setDefaultObservedScales(
          problem,
          scales.observed
        );
    ...
    describe('filterScenariosWithResults', function () {
      ...
      it('should only return the scenarios in which all criteria have fully defined pvfs', function () {
        ...
        var result = workspaceService.filterScenariosWithResults(
          problem,
          subProblem,
          scenarios
        );
    ...
    describe('validateWorkspace', function () {
      it('should indicate which required properties are missing', function () {
        ...
        expect(validity.errorMessage).toBe(
          'Missing workspace properties: title, criteria, alternatives, performanceTable'
        );
      });
      ...
      describe('for absolute performances', function () {
        ...
        it('should fail if a required property is missing', function () {
          ...
          var validityWithoutTitle = workspaceService.validateWorkspace(
            withoutTitle
          );
          expect(validityWithoutTitle.isValid).toBeFalsy();
          expect(validityWithoutTitle.errorMessage).toBe(
            'Missing workspace property: title'
          );
          var withoutCriteriaAndAlternatives = _.omit(example, [
            'criteria',
            'alternatives'
          ]);
          var validitywithoutCriteriaAndAlternatives = workspaceService.validateWorkspace(
            withoutCriteriaAndAlternatives
          );
          expect(validitywithoutCriteriaAndAlternatives.isValid).toBeFalsy();
          expect(validitywithoutCriteriaAndAlternatives.errorMessage).toBe(
            'Missing workspace properties: criteria, alternatives'
          );
        });
        ...
        it('should fail if there are fewer than 2 criteria', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            exampleWithOneCriterion
          );
          expect(validity.isValid).toBeFalsy();
        ...
        it('should fail if there are fewer than 2 alternatives', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            exampleWithOneAlternative
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Two or more alternatives required'
          );
        });
        ...
        it('should fail if there is data in the performanceTable for nonexistent criteria', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            exampleWithExtraPerformanceData
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Performance table contains data for nonexistent criterion: "nonsense"'
          );
        });
        ...
        it('should fail if there is data in the performanceTable for nonexistent alternatives', function () {
          ...
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Performance table contains data for nonexistent alternative: "nonsense"'
          );
        });
        ...
        it('should fail if a cell of the performance table is left empty', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            exampleWithMissingPerformanceData
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Performance table is missing data for criterion "Bleed" and alternative "Enox"'
          );
        });
        ...
        it('should fail if a criterion lacks a title', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            missingCriterionTitle
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Missing title for criterion: "Bleed"'
          );
        });
        ...
        it('should fail if a alternative lacks a title', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            missingAlternativeTitle
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Missing title for alternative: "Hep"'
          );
        });
      });
      ...
      describe('for relative performances', function () {
        ...
        it('should fail when the baseline is missing', function () {
          var problemWithMissingBaseline = _.cloneDeep(
            exampleRelativeProblem()
          );
          delete problemWithMissingBaseline.performanceTable[0].performance
            .distribution.parameters.baseline;
          var validity = workspaceService.validateWorkspace(
            problemWithMissingBaseline
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Missing baseline for criterion: "crit1"'
          );
        });
        ...
        it('should fail when the mu refers to a nonexistent alternative', function () {
          ...
          var mu =
            problemWithNonsenseMu.performanceTable[0].performance.distribution
              .parameters.relative.mu;
          delete mu['4939'];
          mu.nonsense = 3;
          var validity = workspaceService.validateWorkspace(
            problemWithNonsenseMu
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'The mu of the performance of criterion: "crit1" refers to nonexistent alternative: "nonsense"'
          );
        });

        it('should fail when the cov rownames or colnames refer to a nonexistent alternative', function () {
          var problemWithNonsenseRowName = _.cloneDeep(
            exampleRelativeProblem()
          );
          var cov =
            problemWithNonsenseRowName.performanceTable[0].performance
              .distribution.parameters.relative.cov;
          cov.rownames[0] = 'nonsense';
          var validity = workspaceService.validateWorkspace(
            problemWithNonsenseRowName
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'The covariance matrix of criterion: "crit1" refers to nonexistent alternative: "nonsense"'
          );
          var problemWithNonsenseColName = _.cloneDeep(
            exampleRelativeProblem()
          );
          cov =
            problemWithNonsenseColName.performanceTable[0].performance
              .distribution.parameters.relative.cov;
          cov.colnames[0] = 'nonsense';
          validity = workspaceService.validateWorkspace(
            problemWithNonsenseColName
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'The covariance matrix of criterion: "crit1" refers to nonexistent alternative: "nonsense"'
      ...
      describe('regarding preferences', function () {
        it('should fail if a preference refers to a nonexistent criterion', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            exampleWithBadPreference
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Preferences contain data for nonexistent criterion: "nonsense"'
          );
        });
        ...
        it('should fail if there are mixed preferences', function () {
          ...
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Preferences should all be the same type'
          );
        });
        ...
        it('should fail if ordinal preferences are inconsistent because one is compared to multiple', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            inconsistentOrdinalWithTree
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Inconsistent ordinal preferences'
          );
        });
        ...
        it('should fail if ordinal preferences are inconsistent', function () {
          ...
          validity = workspaceService.validateWorkspace(
            inconsistentOrdinalSelfReference
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Inconsistent ordinal preferences'
        ...
        it('should fail on inconsistent exact preferences', function () {
          ...
          var validity = workspaceService.validateWorkspace(
            inconsistentWrongRoot
          );
          expect(validity.isValid).toBeFalsy();
          expect(validity.errorMessage).toBe(
            'Inconsistent exact weighting preferences'
          );
    ...
    describe('percentifyScales', function () {
      it('should convert proportions to percentages where possible, ignoring scales for excluded datasources', function () {
        ...
        var result = workspaceService.percentifyScales(
          criteria,
          observedScales
        );
        var expectedResult = {
    ...
    describe('checkForMissingValuesInPerformanceTable', function () {
      it('should return true if there are missing values for both the effect and distribution', function () {
        ...
        var result = workspaceService.checkForMissingValuesInPerformanceTable(
          performanceTable
        );
      ...
      it('should return true if the distribution has a missing value and there is no effect', function () {
        var performanceTable = [
          {
            performance: {
              distribution: {
