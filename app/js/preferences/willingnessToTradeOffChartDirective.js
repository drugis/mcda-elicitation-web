'use strict';
define(['c3', 'd3', 'lodash'], function (c3, d3, _) {
  var dependencies = ['$timeout', 'TradeOffService', 'significantDigits'];
      ...
      link: function (scope, element) {
        ...
        scope.$watch(
          ...
          function (newSettings) {
            if (
              newSettings &&
              newSettings.firstCriterion.dataSources[0].pvf.range
            ) {
        ...
        function updateFirstPoint() {
          scope.areCoordinatesSet = TradeOffService.areCoordinatesSet(
            scope.coordinates
          );
          if (scope.areCoordinatesSet) {
            scope.coordinates.x = _.clamp(
              scope.coordinates.x,
              scope.sliderOptions.floor,
              scope.sliderOptions.ceil
            );
            scope.coordinates.y = _.clamp(
              scope.coordinates.y,
              scope.minY,
              scope.maxY
            );
        ...
        function redrawPlot() {
          ...
          chart.load(data);
          TradeOffService.getIndifferenceCurve(
            scope.problem,
            criteria,
            scope.coordinates
        ...
        function updateSecondPoint() {
          ...
          scope.sliderOptions.maxLimit =
            data.columns[2][data.columns[2].length - 1];
          var secondPointCoordinates = TradeOffService.getYValue(
            scope.coordinates.x2,
            data.columns[2],
            data.columns[3]
          );
        ...
        function initChart(newSettings) {
          ...
          scope.coordinates = {};
          scope.sliderOptions.floor = significantDigits(
            newSettings.firstCriterion.dataSources[0].pvf.range[0]
          );
          scope.sliderOptions.ceil = significantDigits(
            newSettings.firstCriterion.dataSources[0].pvf.range[1]
          );
          scope.sliderOptions.step = significantDigits(
            (scope.sliderOptions.ceil - scope.sliderOptions.floor) / 100
          );
          ...
          var initialSettings = TradeOffService.getInitialSettings(
            root,
            data,
            coordRanges,
            newSettings
          ...
          function clickHandler() {
            ...
            scope.coordinates.x = significantDigits(
              chart.internal.x.invert(coords[0])
            );
            scope.coordinates.y = significantDigits(
              chart.internal.y.invert(coords[1])
            );
            updateFirstPoint();
        ...
        function updateAxisLabels() {
          scope.units = {
            x:
              scope.settings.firstCriterion.dataSources[0].unitOfMeasurement
                .label,
            y:
              scope.settings.secondCriterion.dataSources[0].unitOfMeasurement
                .label
          };
