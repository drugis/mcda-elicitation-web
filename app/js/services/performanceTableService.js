      ...
      return _.reduce(
        ...
        function (accum, entry) {
          if (
            ...
            var factor =
              dataSource.unitOfMeasurement.type === 'percentage' ? 100 : 1;
        ...
        function (accum, entry) {
          if (hasRangeDistribution(entry, dataSource.id)) {
            var factor =
              dataSource.unitOfMeasurement.type === 'percentage' ? 100 : 1;
            accum.push(
              entry.performance.distribution.parameters.lowerBound * factor
            );
            accum.push(
              entry.performance.distribution.parameters.upperBound * factor
            );
          }
          return accum;
        },
