...
module.exports = function (db) {
  function get(workspaceId, callback) {
    ...
    const query =
      'SELECT id, owner, state FROM inProgressWorkspace WHERE id = $1';
    db.query(query, [workspaceId], function (error, result) {
      if (error) {
        ...
        callback({
          message:
            'In progress workspace with ID ' + workspaceId + ' not found.',
          statusCode: 404
        });
      } else {
  ...
  function create(ownerId, state, callback) {
    ...
    const query =
      'INSERT INTO inProgressWorkspace (owner, state) VALUES ($1, $2) RETURNING id';
    db.query(query, [ownerId, state], function (error, result) {
  ...
  function update(state, workspaceId, callback) {
    ...
    db.query(query, [state, workspaceId], callback);
  }

  function query(ownerId, callback) {
    logger.debug('getting in progress workspaces');
    const query =
      'SELECT id, owner, state, title FROM inProgressWorkspace WHERE owner = $1';
    db.query(query, [ownerId], function (error, result) {
