...
module.exports = function (db) {
  ...
  function create(client, owner, title, problem, callback) {
    logger.debug('creating workspace');
    const query =
      'INSERT INTO workspace (owner, title, problem) VALUES ($1, $2, $3) RETURNING id';
    client.query(query, [owner, title, problem], function (error, result) {
  ...
  function getWorkspaceInfo(client, workspaceId, callback) {
    logger.debug('getting workspace info');
    const query =
      'SELECT id, owner, problem, defaultSubproblemId as "defaultSubProblemId", defaultScenarioId AS "defaultScenarioId" FROM workspace WHERE id = $1';
    client.query(query, [workspaceId], function (error, result) {
      if (error) {
        callback(error);
      } else if (!result.rows.length) {
